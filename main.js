        ...
        if (regexp.test(file.name)) {
            ...
            for (var i = 0; i < name.length; i++) {
                var temp = he.encode('<h2 style="color:red;font-size: 24px;"><strong>Old ' +
                    name[i] + '</strong></h2>');
                content_collector += temp;
                var new_information = name[i];
                // this line fishes out the needed tag and gets the text out of it
                to_check_raw = file.dom(xml_tags[i]).text();
                if (to_check_raw !== '') {
                    content_collector += to_check_raw;
            ...
            file.dom(xml_tags[0]).text(content_collector, {
